...
const MovieDetailScreen = ({ route, navigation }) => {
    ...
    const getLocalPath = (url) => {
        if (!url) return '';
        const filename = getMovieBlastLocalPathKey(url);
    ...
    const handleDownload = async (source) => {
        const rawUrl = source?.link;
        const statusKey = rawUrl;
        const path = getLocalPath(rawUrl);
        const unsupportedMode = getUnsupportedPlaybackMode(source);

        if (unsupportedMode) {
            Alert.alert('Unsupported Source', 'This source requires embed or supported-host resolution that is not available in this app yet.');
            return;
        }
        ...
        setDownloadStatus(prev => ({
            ...prev,
            [statusKey]: { ...prev[statusKey], isDownloading: true, progress: 0 }
        }));
        ...
        try {
            ...
            const fakeInterval = setInterval(() => {
                ...
                setDownloadStatus(prev => {
                    const current = prev[statusKey] || {};
                    // Only update if we haven't received real progress events (contentLength > 0)
                    if (!current.hasRealProgress) {
                        return {
                            ...prev,
                            [statusKey]: { ...current, isDownloading: true, progress: fakeProgress, hasRealProgress: false }
                        };
                    }
                    return prev;
                });
                ...
                progress: (res) => {
                    ...
                    if (res.contentLength > 0) {
                        ...
                        if (now - lastUpdate > 500 || progress === 100) {
                            ...
                            setDownloadStatus(prev => ({
                                ...prev,
                                [statusKey]: { ...prev[statusKey], isDownloading: true, progress, hasRealProgress: true }
                            }));
                        }
                    }
            ...
            setDownloadStatus(prev => ({
                ...prev,
                [statusKey]: { isDownloaded: true, isDownloading: false, progress: 100 }
            }));
            ...
            setDownloadStatus(prev => ({
                ...prev,
                [statusKey]: { ...prev[statusKey], isDownloading: false }
            }));
        }
    };
                                ...
                                <View key={index} style={styles.sourceCard}>
                                    ...
                                    <View style={styles.actionButtons}>
                                        {status.isDownloading ? (
                                            ...
                                            <TouchableOpacity onPress={() => handleDownload(source)}>
